...
inline int DImage::getPixelGray(int x, int y){
  int idx;
  if((x >= _w) || (y >= _h) || (x < 0) || (y < 0)){
    fprintf(stderr, "DImage::getPixelGray(%d,%d) out of bounds w=%d h=%d\n",
	    x, y, _w, _h);
    return -1;
    // abort();
  }
  idx = (_w * y + x);
  if(_imgType == DImage_RGB){
    fprintf(stderr, "DImage::getPixelGray() function called for"
	    " RGB image\n");
    abort();
